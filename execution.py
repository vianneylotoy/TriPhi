...
class Execution:
	...
	#Sort according to φi the largest in descending order
	def sort_φi(self):
		self.LT_sorted = sorted(self.LT, key=lambda varphi: varphi[1], reverse = True)
	...
	#Display of the list of tasks sorted according to φi the largest
	def display_sortedTask(self):
		self.doc.write("___________________________________________________ Task sorting by φi ______________________________________________________"+'\n')
		for i,ti in enumerate(self.LT_sorted):
			self.tache.pi, self.tache.φi, self.tache.si = ti
			print("Task sorting by φi ->: T[%r] = [%r,%r,%r]" % (i,self.tache.pi, self.tache.φi, self.tache.si))	
			self.doc.write("Task sorting by φi->: T["+str(i)+"] = p"+str(i)+"-> "+str(self.tache.pi)+", φ"+str(i)+"-> "+str(self.tache.φi)+", s"+str(i)+"-> "+str(self.tache.si)+ '\n')
